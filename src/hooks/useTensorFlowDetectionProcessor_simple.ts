...
function isWalkingHazard(box: number[], confidence: number, frameWidth = 320, frameHeight = 320): boolean {
  ...
  if (confidence < 0.55) return false // Reduced from 0.65 to 0.55
  
  // 2. More permissive size filtering 
  if (area < 0.002) return false // Very small objects only (< 0.2% of frame)
  if (area > 0.9) return false   // Only reject obviously full-screen detections
  ...
  if (!isInReasonableArea) return false
  ...
  if (aspectRatio > 5.0) return false // Only reject extremely wide objects (was 3.0)
...
export function useTensorFlowDetectionProcessor({
  ...
  React.useEffect(() => {
    ...
    // Step 2: Find closest obstacle (highest height = closest distance)
    if (currentDetections.length > 0) {
      ...
      // Convert to distance and report
      ...
      if (currentTime - lastDetectionTime.current >= cooldownMs) {
        console.log(`[Simple Detection] Closest obstacle: ${(distance * 100).toFixed(0)}cm distance, ${(closestObstacle.confidence * 100).toFixed(1)}% confidence`)
